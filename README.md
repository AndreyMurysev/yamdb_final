...
```
Данный проект создан с целью изучения DevOps и фреймворка Django REST framework для разработки API., но при этом 
данное приложение возможно использовать и в действующих проектах, (в часности написанных на React) где есть потребность 
в ранжировании продукта, который предоставляет ваш сервис, по оценкам пользователей. Дополнительным плюсом будет 
настройка Action для командной работы, которая позволит упростить работу, как во время работы над проектам, путем 
предварительного прохождения тестов при пуше в GIT, так и на уровне продакшена, для оперативного размещения проекта на сервере.
```
...
- После успешного деплоя зайдите на боевой сервер и выполните команды миграции, создания суперпользователя и сбора статики (опиcаны ранее):
